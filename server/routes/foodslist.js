...
router.post("/id", async (req, res, next) => {
  ...
  try {
    ...
    if (checkFdcFoodIdInDb.length) {
      ...
      // import category
      const checkCategoryDb = await trx("category")
        .select("id", "fdc_id_category")
        .whereIn("fdc_id_category", [
          FoodData[0].wweiaFoodCategory.wweiaFoodCategoryCode,
        ]);
      let dbInsertCategory;
      if (!checkCategoryDb.length) {
        const objForCategory = {
          food_category:
            FoodData[0].wweiaFoodCategory.wweiaFoodCategoryDescription,
      ...
      const objForFoods = {
        ...
        fdc_id: fdcFoodId,
        category_id:
          (dbInsertCategory !== undefined ? dbInsertCategory[0] : false) ||
          checkCategoryDb[0].id,
      };
      ...
      const checkmeasureUnitsDb = await trx("measure_units")
        .select("id", "fdc_id")
        .whereIn("fdc_id", idsMeasureUnits);
      // filter ids
      const arrayofobjM = objmeasureUnitsAndPortion.map((m) => ({
        fdc_id: m.fdc_id,
        measure_unit_name: m.measure_unit_name,
      }));
      const filterobjM = arrayofobjM.filter(
        (m) => !checkmeasureUnitsDb.find((x) => x.fdc_id === m.fdc_id)
      );

      // insert
      const insertMeasureUnits = await trx("measure_units").insert(
        filterobjM,
        "id"
      );
      const objP = [
        ...checkmeasureUnitsDb,
        ...filterobjM.map((mesureU, idx) => ({
          ...mesureU,
          id: insertMeasureUnits[idx],
        })),
      ];
      ...
      // filter nutrients
      const mapDbNutirents = new Map(
        getdbnutrients.map((n) => [n.fdc_id, n.id])
      );
      const filterN = FoodData[0].foodNutrients.filter((e) =>
        mapDbNutirents.has(e.nutrient.id)
      );
      // obj preper
      ...
      const insertDbN = await trx("food_nutrients").insert(
        fdcNutreintIds,
        "id"
      );
